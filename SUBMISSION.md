...
Everything needed to submit **Steil 3D Configurator** is prepared. This file is
...
- `steil-3d-configurator.php` — complete plugin header, GPLv2-or-later, text
  domain `steil-3d-configurator`.
...
npm run plugin-zip    # → steil-3d-configurator.zip
```
...
3. Upload `steil-3d-configurator.zip`.
...
You receive an SVN repo at `https://plugins.svn.wordpress.org/steil-3d-configurator/`.

```bash
svn co https://plugins.svn.wordpress.org/steil-3d-configurator/ svn-steil
cd svn-steil

# Plugin files go in trunk/ (copy the built plugin contents, not the repo root)
#   trunk/  ← steil-3d-configurator.php, includes/, build/, src/, assets/, readme.txt, LICENSE, ...
# Then tag the release:
svn cp trunk tags/0.0.1

# Directory page assets go in assets/ (NOT trunk):
cp ../steil-3d-configurator/.wordpress-org/icon-256x256.png   assets/
cp ../steil-3d-configurator/.wordpress-org/banner-772x250.png assets/
cp ../steil-3d-configurator/.wordpress-org/banner-1544x500.png assets/
cp ../steil-3d-configurator/.wordpress-org/screenshot-1.png   assets/
cp ../steil-3d-configurator/.wordpress-org/screenshot-2.png   assets/

svn add --force trunk assets tags
svn ci -m "Steil 3D Configurator 0.0.1"
```
